...
- **Reusability over Duplication:** Duplicate code is a liability. Extract repeating behaviors into clean utility functions, custom React hooks, or Laravel traits.
...
- **Composition over Inheritance:** In both React components and Laravel services, favor composing behaviors together over building deep class inheritance trees.
- **Fail Fast:** Validate inputs and authorization parameters immediately. Throw exceptions or return early responses as soon as a check fails to prevent unnecessary processing.
- **Explicit over Implicit:** Explicit code is easier to debug. Do not use magic methods, dynamic properties, or implicit type casting.
- **Convention over Configuration:** Follow Laravel's standard folder conventions and React's atomic layout guidelines to reduce configuration overhead.

---

## 3. Frontend Implementation Rules (React + TypeScript)
...
- Export components as default exports from their respective files, and use `index.ts` files inside directories for clean importing.

### 3.2. TypeScript Standards
- Enable `strict` mode in `tsconfig.json`.
- Never use `any`. Define explicit `interface` or `type` contracts for all variables, parameters, and API responses.
- Use Discriminated Unions for handling polymorphic metadata states returned by the ACA.
...
- Standardize on **React Hook Form** paired with **Zod** schema validation.
...
## 4. Backend Implementation Rules (Laravel 12)

### 4.1. Thin Controllers
- Controllers must contain **zero business logic**.
- A controller's only responsibility is to:
  1. Capture the validated input using a Form Request.
  2. Call the corresponding Service class method.
  3. Format and return the result using an API Resource.
- Limit controllers to standard RESTful resource methods: `index`, `show`, `store`, `update`, `destroy`.

### 4.2. Service Layer
- All business operations, image resizing, and file operations must live inside dedicated Service classes.
- Services should be constructor-injected into controllers to promote decoupling and testing mockability.
- Use Laravel's database transactions (`DB::transaction(fn() => ...)`) inside services for operations that update multiple tables (e.g., creating a potential and its location, parsing Excel sheets) to guarantee atomic writes.

### 4.3. Models & Scopes
- All database keys must use UUIDs (UUID v4) generated automatically via a `HasUuid` trait on model creation.
- Define relationship structures explicitly using strict return types (e.g., `BelongsTo`, `HasMany`, `BelongsToMany`).
- Use query scopes for reusable constraints (e.g., `scopePublished`, `scopeFeatured`, `scopeInCategory`).
- Enable Soft Deletes on the `Potential` model to retain historical records.

### 4.4. Requests & Validation
- Validate all incoming data inside dedicated Form Request classes. Never write validation logic directly in controller methods.
...
- All API exceptions (e.g., ModelNotFound, ValidationException, AuthenticationException) must be caught by a global exception handler and formatted into the standard JSON error response:
  ```json
  {
    "success": false,
    "error": {
  ...
  ```
- Use descriptive custom exceptions (e.g., `ImportValidationException`, `MediaProcessingException`) to handle business logic failures.

### 4.6. File Storage & Media Processing
- Media operations must pass through the `ImageProcessingService`.
- Automatically compress uploaded images to WebP format, cap resolution at 1200px width, and restrict uploads to a 5MB maximum file size.
- Store files under `storage/app/public/uploads` and generate URLs using `Storage::url()`.
...
- Use Laravel Migrations to declare database updates. Never execute manual raw SQL adjustments on production databases.
- Ensure all foreign keys define explicit cascade constraints:
...
- **SQL Injection Prevention:** Use Eloquent parameter binding. Never construct SQL strings manually.
...
- **TypeScript:** Strict mode compile check (`tsc --noEmit`) must return zero errors.
- **ESLint & Prettier:** Frontend code must format to standard JS/TS rules (2 spaces indentation, no unused imports, single quotes).
- **Laravel Pint:** Backend PHP code must pass Pint formatting to match PSR-12 style rules.
- **No Dead Code:** Clean out unused variable declarations, debug logs (`console.log`, `dd()`, `dump()`), and commented-out code blocks before submitting code.
...
- [ ] Are all inputs validated through Form Requests or Zod schemas?
