...
const CARD_SHADOW = '0 18px 48px rgba(15,61,52,0.08)';
const CARD_SHADOW_HOVER = '0 24px 70px rgba(15,61,52,0.12)';
const TILT_MAX = 2;
...
function CategoryBadge({ label, slug, colorCode }: { label: string; slug: string; colorCode: string | null }) {
  return (
    <span
    ...
    >
      <span
        ...
        style={{ backgroundColor: colorCode ?? '#16a34a' }}
        aria-hidden="true"
      />
...
function GlassReflection({ enabled }: { enabled: boolean }) {
  if (!enabled) return null;

  return (
    <div className="pointer-events-none absolute inset-0 z-20 overflow-hidden rounded-[32px]" aria-hidden="true">
      <motion.div
...
function FeaturedPotentialCardContent({ item, variant }: FeaturedPotentialCardProps) {
  ...
  const cardTransform = enableTilt
    ? `perspective(1200px) rotateX(${tilt.rotateX}deg) rotateY(${tilt.rotateY}deg) translateY(${isHovered ? -4 : 0}px) scale(${isHovered ? 1.01 : 1})`
    : enableHoverLift
      ? `translateY(${isHovered ? -4 : 0}px) scale(${isHovered ? 1.01 : 1})`
      : undefined;

  const cardTransition = isTilting
    ? 'box-shadow 240ms ease-out, transform 240ms ease-out'
    : 'transform 240ms ease-out, box-shadow 240ms ease-out';

  return (
    <Link
      to={`/potentials/${item.category.slug}/${item.slug}`}
      className="group block h-full cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[--border-focus] focus-visible:ring-offset-2 rounded-[32px]"
    ...
    >
      ...
      >
        <SpotlightSurface
          ...
          className={`relative w-full overflow-hidden rounded-[32px] ${aspectClass}`}
          style={{
        ...
        >
          <img
            ...
            alt={item.title}
            className="absolute inset-0 h-full w-full object-cover transition-transform duration-[700ms] ease-out [@media(hover:hover)]:group-hover:scale-[1.02]"
            loading="lazy"
            decoding="async"
          />
...
export function FeaturedLargeCard({ item }: { item: PotentialListItem }) {
  return <FeaturedPotentialCardContent item={item} variant="large" />;
}

export function FeaturedSmallCard({ item }: { item: PotentialListItem }) {
  return <FeaturedPotentialCardContent item={item} variant="small" />;
}

function FeaturedCardSkeleton({ variant }: { variant: 'large' | 'small' }) {
  const aspectClass = variant === 'large' ? 'aspect-[4/3]' : 'aspect-video';
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      <Skeleton className="absolute inset-0 h-full w-full rounded-[32px]" />
