...
import { mockPotentials } from '@/dashboard/components/data/mockData';
...
export default function PotentialsPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return mockPotentials.filter((item) => item.name.toLowerCase().includes(normalized));
  }, [query]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Potentials"
        description="Shared table infrastructure for future content management modules."
        badge="Foundation"
        actions={<DashboardButton variant="secondary">Create Draft</DashboardButton>}
      />

      <DashboardDataTable
        title="Potentials workspace"
        description="Reusable table shell demonstrating search, selection, and row actions without CRUD logic."
        rows={pagedRows}
        ...
        toolbar={
          <div className="space-y-3">
            ...
            <TableToolbar
              ...
              actions={
                <>
                  <TableFilters filters={<><TableSearch value={query} onChange={setQuery} placeholder="Search potentials" /><select className="rounded-full border border-[#e6eae9] bg-white px-3 py-2 text-sm text-[#0f1720]"><option>Status</option></select></>} onReset={() => setQuery('')} />
                  <ColumnVisibilityMenu>
                    <span className="rounded-full border border-[#e6eae9] bg-[#f8faf9] px-2.5 py-1 text-xs">Name</span>
                    <span className="rounded-full border border-[#e6eae9] bg-[#f8faf9] px-2.5 py-1 text-xs">Status</span>
                  </ColumnVisibilityMenu>
                </>
              }
            />
