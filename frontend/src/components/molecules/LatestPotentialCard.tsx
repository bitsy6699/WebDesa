...
export interface LatestPotentialCardProps {
  item: PotentialListItem;
}

const CARD_SHADOW = '0 16px 42px rgba(15,61,52,0.08)';
...
function getCategoryBadgeStyle(slug: string) {
  return CATEGORY_BADGE[slug] ?? { bg: 'rgba(22,163,74,0.18)', text: '#16a34a' };
}

function formatPublishDate(createdAt?: string): string | null {
...
function CategoryBadge({ label, slug }: { label: string; slug: string }) {
...
export const LatestPotentialCard = memo(
  function LatestPotentialCard({ item }: LatestPotentialCardProps) {
    ...
    return (
      <Link
        to={detailHref}
        className="group flex h-full cursor-pointer flex-col rounded-[28px] focus:outline-none focus-visible:ring-2 focus-visible:ring-[--border-focus] focus-visible:ring-offset-2"
      ...
      >
        <motion.article
          className={[
            ...
            enableHoverMotion
              ? 'transition-[transform,box-shadow] duration-240 ease-out [@media(hover:hover)]:group-hover:-translate-y-1 [@media(hover:hover)]:group-hover:scale-[1.01] [@media(hover:hover)]:group-hover:shadow-[0_20px_60px_rgba(15,61,52,0.12)]'
              : '',
          ].join(' ')}
        ...
        >
          <SpotlightSurface className="relative aspect-video shrink-0 overflow-hidden rounded-t-[28px] bg-[#E8EEEB]" disabled={!enableHoverMotion} spotlightClassName="opacity-80">
            <img
              src={item.cover_image_url ?? placeholderCard}
              alt={item.title}
              className="h-full w-full object-cover transition-transform duration-[600ms] ease-out [@media(hover:hover)]:group-hover:scale-[1.02]"
              loading="lazy"
              decoding="async"
            />
          ...
          <div className="flex flex-1 flex-col gap-3 p-5">
            ...
            <h3 className="line-clamp-2 text-lg font-bold leading-snug text-[#163A35]">
              {item.title}
            </h3>

            {item.short_description && (
              <p className="line-clamp-2 text-sm leading-relaxed text-[#667085]">
                {item.short_description}
              </p>
            )}
            ...
            <div className="mt-auto pt-2">
              <span
                className="inline-flex items-center gap-2 rounded-full px-4 py-2.5 text-sm font-semibold text-[#163A35] transition-colors duration-300 ease-out [@media(hover:hover)]:group-hover:bg-[var(--color-primary)] [@media(hover:hover)]:group-hover:text-white"
                style={{
...
export function LatestPotentialCardSkeleton() {
  return (
    <div
      ...
      aria-busy="true"
    >
      <div className="relative aspect-video shrink-0 overflow-hidden rounded-t-[28px]">
